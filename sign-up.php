...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $contrasena = isset($_POST['password']) ? password_hash($_POST['password'], PASSWORD_BCRYPT) : '';
    $terminos = isset($_POST['condition']) ? $_POST['condition'] : '';

    // Validar que los campos no estén vacíos
    if (!empty($nombre) && !empty($correo) && !empty($contrasena) && !empty($terminos)) {
        ...
        try {
            if ($stmt->execute(['nombre' => $nombre, 'correo' => $correo, 'contrasena' => $contrasena])) {
                // Registro exitoso
                $mensaje = "<div style='color: green; font-weight: bold; margin-top: 20px;'>Registro exitoso. Serás redirigido al login.</div>";
                header("Location: login.php");
                exit();
            } else {
                $mensaje = "<div style='color: red; font-weight: bold; margin-top: 20px;'>Error en el registro.</div>";
            }
                    ...
                    <div class="">
                        <div class="d-flex justify-content-between gap-2">
                            <div class="form-check style-check d-flex align-items-start">
                                <input class="form-check-input border border-neutral-300 mt-4" type="checkbox" name="condition" value="1" id="condition" required>
