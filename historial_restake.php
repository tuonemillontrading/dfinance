...
// Obtener el historial de depósitos y restakes del usuario
$query = "
    SELECT id, user_id, tipo_operacion, sistema, cantidad, fecha
    FROM historial 
    WHERE user_id = :user_id AND (tipo_operacion = 'deposito' OR tipo_operacion = 'reinvestment')
    ORDER BY fecha DESC";
                ...
                <?php if (count($historial) > 0): ?>
                    <?php foreach ($historial as $transaccion): ?>
                        <tr style="font-size: 12px; background-color: transparent;">
                            ...
                            <td><?php echo $transaccion['tipo_operacion'] == 'reinvestment' ? 'Restake' : 'Deposito'; ?></td>
